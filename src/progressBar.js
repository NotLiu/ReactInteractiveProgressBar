...
export const ProgressNotch = styled.div`
  position: absolute;
  left: ${(props) => props.progressPercentage || "0%"};
  top: -100%;
...
export default function ProgressBar(props) {
  ...
  const [progressEmpty, setProgressempty] = useState(props.progressEmpty);
  const [lastSegment, setlastSegment] = useState(props.fill);
  const [secondlastSegment, setSecondlastSegment] = useState(props.fill);
